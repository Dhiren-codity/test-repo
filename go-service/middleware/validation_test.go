...
func TestValidateParseRequest_BasicScenarios(t *testing.T) {
	tests := []struct {
		name         string
		content      string
		path         string
		...
		},
		{
			...
			wantErrCount: 1,
			wantFields:   []string{"content"},
			wantReasons:  []string{"exceeds maximum size"},
		},
		{
		...
		},
		{
			...
			content:      "abc",
			path:         strings.Repeat("a", MaxPathLength-5) + "../x",
			wantErrCount: 2,
			wantFields:   []string{"path", "path"},
			wantReasons:  []string{"maximum length", "directory traversal"},
		},
		{
			name:         "no error",
	...
	// Ensure clean global state not asserted here but keep clean for other tests
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i, r := range tt.wantReasons {
				if i < len(errs) {
					assert.Contains(t, errs[i].Reason, r)
